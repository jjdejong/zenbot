...
module.exports = {
  ...
  onPeriod: function (s, cb) {
    ...
    .then(function(result) {
        ...
        s.signal = null
        if (_switch != 0) {
          if (s.period.srsi_D > s.options.stoch_k_sell
			  && s.period.srsi_K <= s.period.srsi_D) {
            s.signal = 'sell'
          } else if (s.period.srsi_D < s.options.stoch_k_buy
  ...
  onReport: function (s) {
    ...
    cols.push(z(8, n( s.period.srsi_D).format('0.00000').substring(0,7), ' ').cyan)
    cols.push(z(8, n(s.period.srsi_K).format('0.00000').substring(0,7), ' ').cyan)
